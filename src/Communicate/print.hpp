...
#else
#define HPX_CORE_EXPORT
#define hexuint32(a)     hpx::debug::hex<4>(a) << " "
#define hexpointer(a)    hpx::debug::hex<8>(a) << " "
#define decnumber(a)     hpx::debug::dec<4>(a) << " "
#define LOG_TRACE_MSG(a) std::cout << "<TRC> " << a << std::endl;
#define LOG_ERROR_MSG(a) std::cout << "<ERR> " << a << std::endl;
#define LOG_EXCLUSIVE(a)
#define LOG_STUFF_ENABLED 1
#endif
...
#define HPX_DP_LAZY(printer, Expr) \
    printer.eval([&] {             \
        return Expr;               \
    })
#define HPX_DP_ONLY(printer, Expr)        \
    if constexpr (printer.is_enabled()) { \
        printer.Expr;                     \
    };

#define NS_DEBUG hpx::debug
...
namespace hpx::debug {
    ...
    struct ptr {
        ...
        HPX_CORE_EXPORT friend std::ostream& operator<<(std::ostream& os, ptr const& d);
    };
    ...
    struct ipaddr {
        ...
        HPX_CORE_EXPORT friend std::ostream& operator<<(std::ostream& os, ipaddr const& p);
    };
    ...
    namespace detail {
        ...
        struct current_time_print_helper {
            HPX_CORE_EXPORT friend std::ostream& operator<<(std::ostream& os,
                                                            current_time_print_helper);
        };
    }  // namespace detail
    ...
    // ------------------------------------------------------------------
    [[nodiscard]] constexpr std::uint32_t crc32(void const*, std::size_t) noexcept {
        return 0;
    }
    ...
    // ------------------------------------------------------------------
    struct mem_crc32 {
        HPX_CORE_EXPORT mem_crc32(void const* a, std::size_t len) noexcept;

        std::uint64_t const* addr_;
        std::size_t const len_;

        HPX_CORE_EXPORT friend std::ostream& operator<<(std::ostream& os, mem_crc32 const& p);
    };
    ...
    struct enable_print<false> {
        ...
        template <typename T>
        static constexpr void array(std::string const&, std::vector<T> const&) noexcept {}

        template <typename T, std::size_t N>
        static constexpr void array(std::string const&, std::array<T, N> const&) noexcept {}

        template <typename T>
        static constexpr void array(std::string const&, T const*, std::size_t) noexcept {}
        ...
        [[nodiscard]] static constexpr bool declare_variable(Args const&...) noexcept {
            return true;
        }
        ...
        template <typename... Args>
        [[nodiscard]] static constexpr empty_timed_var make_timer(const double, Args const&...) {
    ...
    public:
        ...
        [[nodiscard]] static constexpr bool is_enabled() noexcept { return true; }
        ...
        template <typename T>
        static void array(std::string const& name, T const* data, std::size_t size) {
        ...
        template <typename... Args>
        [[nodiscard]] static constexpr timed_var<Args...> make_timer(const double delay,
                                                                     const Args... args) {
