...
namespace hpx::debug {
    ...
    namespace detail {
        ...
        template HPX_CORE_EXPORT void print_dec(std::ostream&, bool const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::int16_t const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::uint16_t const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::int32_t const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::uint32_t const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::int64_t const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::uint64_t const&, int);

        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::atomic<int> const&, int);
        template HPX_CORE_EXPORT void print_dec(std::ostream&, std::atomic<unsigned int> const&,
                                                int);
        template HPX_CORE_EXPORT void print_dec(
            std::ostream&, ippl::communication_pool::simple_counter<unsigned int> const&, int);
        template HPX_CORE_EXPORT void print_dec(
            std::ostream&, ippl::communication_pool::simple_counter<int> const&, int);
    }  // namespace detail
    ...
    namespace detail {
        ...
        template HPX_CORE_EXPORT void print_hex(std::ostream&, std::thread::id, int);
        template HPX_CORE_EXPORT void print_hex(std::ostream&, unsigned long, int);
        template HPX_CORE_EXPORT void print_hex(std::ostream&, long, int);
        template HPX_CORE_EXPORT void print_hex(std::ostream&, int, int);
        template HPX_CORE_EXPORT void print_hex(std::ostream&, unsigned int, int);
        template HPX_CORE_EXPORT void print_hex(std::ostream&, void*, int);
        ...
        template HPX_CORE_EXPORT void print_ptr(std::ostream&, int, int);
        template HPX_CORE_EXPORT void print_ptr(std::ostream&, long, int);
    ...
    namespace detail {
        ...
        template HPX_CORE_EXPORT void print_bin(std::ostream&, unsigned char, int);
        template HPX_CORE_EXPORT void print_bin(std::ostream&, std::uint32_t, int);
        template HPX_CORE_EXPORT void print_bin(std::ostream&, std::uint64_t, int);

#if defined(__APPLE__)
        // Explicit instantiation necessary to solve undefined symbol for MacOS
        template HPX_CORE_EXPORT void print_bin(std::ostream&, unsigned long, int);
#endif
    }  // namespace detail
    ...
    std::ostream& operator<<(std::ostream& os, ipaddr const& p) {
        os << std::dec << static_cast<int>(p.data_[0]) << "." << static_cast<int>(p.data_[1]) << "."
           << static_cast<int>(p.data_[2]) << "." << static_cast<int>(p.data_[3]);
        return os;
    }
    ...
    mem_crc32::mem_crc32(void const* a, std::size_t len) noexcept
    ...
    std::ostream& operator<<(std::ostream& os, mem_crc32 const& p) {
        ...
        os << "Memory:";
        os << " address " << hpx::debug::ptr(p.addr_) << " length " << hpx::debug::hex<6>(p.len_)
           << " CRC32:" << hpx::debug::hex<8>(detail::crc32(p.addr_, p.len_)) << "\n";

        auto const max_value =
            (std::min)(static_cast<std::size_t>(std::ceil(static_cast<double>(p.len_) / 8.0)),
...
#endif
                    std::string const temp = "(" + std::to_string(guess_rank()) + ")";
#if defined(HPX_GCC_VERSION) && HPX_GCC_VERSION >= 110000
#pragma GCC diagnostic pop
        ...
        ///////////////////////////////////////////////////////////////////////
        template <typename T>
        HPX_CORE_EXPORT void print_array(std::string const& name, T const* data, std::size_t size) {
            std::cout << str<20>(name.c_str()) << ": {" << debug::dec<4>(size) << "} : ";
            std::copy(data, data + size, std::ostream_iterator<T>(std::cout, ", "));
            std::cout << "\n";
        }

        template HPX_CORE_EXPORT void print_array(std::string const&, std::uint64_t const*,
                                                  std::size_t);
    }  // namespace detail
}  // namespace hpx::debug
/// \endcond
